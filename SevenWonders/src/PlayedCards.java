...
public class PlayedCards implements OnscreenComponent {
  ...
  private BufferedImage renderImage(int width, int height) {
    ...
    int totalWidth =
        (int)
            (numCards * cardWidth * currentScale
                + (numCards - 1) * cardWidth * currentScale * SPACE);
    if (totalWidth <= width) {
    ...
    int maxCardsInGroup =
        1
            + (int)
                Math.max(0, (height - cardHeight * currentScale) / (cardHeight * currentScale / 5));
    ...
    if (totalWidth <= width) {
      ...
      for (int i = 0; i < groups.size(); i++) {
        int totalHeight =
            (int) (currentScale * cardHeight * (1 + (groups.get(i).size() - 1) / 5.0));
        int yOffset = height - totalHeight;
        for (int j = 0; j < groups.get(i).size(); j++) {
          g.drawImage(
              groups.get(i).get(j).getImage(),
              xOffset,
              yOffset,
              (int) (cardWidth * currentScale),
              (int) (cardHeight * currentScale),
  ...
  private void setMaxGroupSize(int maxCardsInGroup, ArrayList<ArrayList<Card>> groups) {
    for (int i = 0; i < groups.size(); i++)
      if (groups.get(i).size() > maxCardsInGroup) {
        ...
        for (int j = 0; j < maxCardsInGroup; j++) newGroup.add(groups.get(i).remove(0));
  ...
  public void renderSelf(Graphics g) {
    ...
    if (direction == Orientation.Up) {
      ...
      g.drawImage(img, renderedRectangle.x, renderedRectangle.y, width, height, null);
    }
  ...
  public void updateMouseLocation(int x, int y) {
    if (!renderedRectangle.contains(x, y)) return;
